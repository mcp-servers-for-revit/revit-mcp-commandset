...
| 元素查询 | ai_element_filter | 16 | 过滤和节点数据验证（含名称关键字和类型ID过滤） |
...
| F-14 | 🔴必测 | 根据类型ElementId查询族实例 | **操作步骤**：1) 先执行F-01查询门实例，记录第一个门的type.typeId（如94654）；2) 使用filterTypeId=94654 + includeInstances=true + includeTypes=false过滤，验证能返回该类型的所有门实例；**验证要点**：返回的所有门实例的type.typeId都应为94654 |
| F-15 | 🔴必测 | 根据类型ElementId查询系统族实例 | **操作步骤**：1) 先执行F-01查询墙体实例，记录第一面墙的type.typeId（如398）；2) 使用filterTypeId=398 + filterCategory="OST_Walls" + includeInstances=true过滤，验证能返回该类型的所有墙体实例；**验证要点**：返回的所有墙体的type.typeId都应为398 |
| F-16 | 🔴必测 | 根据类型ElementId直接查询类型元素 | **操作步骤**：1) 先执行F-01查询墙体实例，记录type.typeId（如398）；2) 使用filterTypeId=398 + includeTypes=true + includeInstances=false过滤，验证能返回该类型元素本身；**验证要点**：返回的应该是类型元素（WallType），其elementId应等于398 |
...
| 场景ID | 优先级 | 预期结果 | 备注 |
|--------|--------|----------|------|
| E-01 | 🔴必测 | 坐标数据被正确消费 | **必须记录**：location坐标值与实际选中位置一致性；**操作要点**：数据流验证：查询墙体geometry→基于坐标进行视觉标记 |
| E-02 | 🟡建议 | 创建删除循环成功 | **操作流程**：①使用create_element创建一面临时墙；②使用ai_element_filter根据创建返回的ID查询该墙体，验证数据正确；③使用operate_element_modify的Delete操作删除该墙；**操作要点**：创建临时墙→查询验证→删除清理 |
| E-03 | 🟡建议 | 隔离状态下的批量操作 | **操作流程**：①使用ai_element_filter查询结构墙（filterCategory="OST_Walls"，可结合filterNameKeyword="结构"）；②使用operate_element_visibility的Isolate操作隔离显示这些墙；③使用operate_element_transform的Move操作批量移动；④使用ResetIsolate恢复显示；**操作要点**：隔离结构墙→批量调整位置→恢复显示 |
| E-04 | 🟡建议 | 建议指导标准化创建 | **操作流程**：①使用get_element_creation_suggestion查询族实例创建参数要求；②根据返回的参数建议，使用create_element批量创建3-5个门或窗实例；③使用operate_element_transform的Flip操作调整朝向；**操作要点**：获取门窗建议→批量创建→检查朝向 |
| E-05 | 🟡建议 | 批量问题标记流程 | **操作流程**：①使用ai_element_filter查询墙体，获取parameters节点中的高度参数；②筛选出高度>5000mm的墙体；③使用operate_element_visual的SetColor操作将这些墙体标记为红色；④记录这些元素的ID和高度形成问题列表；**操作要点**：查找超高构件→红色标记→导出列表 |
| E-06 | 🔵可选 | 容错机制正常 | **前置条件**：准备1个Pin元素（锁定状态）和2个普通元素；**操作流程**：对这3个元素执行operate_element_transform的Move操作；**操作要点**：Pin元素操作失败→自动跳过→继续处理 |
